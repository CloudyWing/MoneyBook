 ...
 */
let LinkedDropDownList = function () {
    ...
    this.create = function () {
        ...
        for (let i = 0; i < dropDownLists.length; i++) {
            $(dropDownLists[i].select).unbind().change({ index: i }, function (e) {
                ...
                dropDownLists[index].hidden.value = $.trim($(this).find('option:selected').text());

                // 選項如果內容有被改變，就將後面下拉式選單的初始值給清空
                for (let j = index + 1; j < dropDownLists.length; j++) {
    ...
    this.refreshItems = function (index) {
        let thisDropDownList = dropDownLists[index] || null;
        let nextDropDownList = dropDownLists[index + 1] || null;
        let prevDropDownList = dropDownLists[index - 1] || null;
        // 記錄前一個下拉選單的值，第二個以後下拉選單才會有作用
        let prevValue = prevDropDownList === null ? '' : prevDropDownList.select.value;
        ...
        // 額外的查詢條件
        for (let paramKey in thisDropDownList.extraParams) {
            let value = thisDropDownList.extraParams[paramKey];

            // 試著從DOM取值
            if (typeof value === 'string' && value !== null) {
                let $elem = thisDropDownList.select.form ?
                ...
                if ($elem.length === 0) {
                    ...
                    if (type === 'radio') {
                        ...
                        postData[paramKey] = $elem.filter(':checked').map(x => x.value);
                    } else {
        ...
        if (prevDropDownList && !thisDropDownList.allowEmptyPrevValue() && prevDropDownList.select.value === '') {
            ...
            $.ajax({
                ...
                if (result.length > 0) {
                    if (thisDropDownList.hasOptionLabel() && result.length > 1) {
                        thisDropDownList.addOptionLabelItem();
                    }

                    for (let i = 0; i < result.length; i++) {
                        thisDropDownList.addItem(result[i]['Text'], result[i]['Value']);
                    }
                }
...
LinkedDropDownList.parse = function (jQuerySelector) {
    let $selects = $(jQuerySelector || 'select')
        .filter('[data-ajax-url]')
        .add($(jQuerySelector).find('[data-ajax-url]'));

    let groups = {};
    $selects.each(function () {
        let group = $(this).data('selectGroup') || '';
        if (!groups[group]) {
            groups[group] = [];
        }
    ...
    for (let group in groups) {
        let dropDownListLists = new LinkedDropDownList();
 ...
 * @property {string} textName - 紀錄選擇Text的hiddenName，對應data-text-name
 * @property {string} defaultValue - 對應data-defalut-value
 * @property {string} optionLabel - 是否要有空白選項，對應data-option-label
 * @property {string} nonUsedCaption - 上層select未選擇時顯示文字，對應allow-non-use-caption
...
let DropDownList = (function () {
     ...
     */
    function DropDownList(select) {
        if (!(this instanceof DropDownList)) {
            return new DropDownList(select);
        }
        ...
        const selectKeys = [
            'ajaxUrl', 'textName', 'defaultValue', 'optionLabel', 'nonUsedLabel', 'changeCallback', 'extraParams'
        ];

        for (let i in selectKeys) {
            let key = selectKeys[i];
            let value = $(select).data(key);
        ...
        function createHidden() {
            $(that.hidden).remove();
            that.hidden = document.createElement('input');
            that.hidden.type = 'hidden';
            that.hidden.name = that.textName;
            $(that.hidden).insertAfter(that.select);
        }
    };

    let _proto = DropDownList.prototype;
    ...
    _proto.addItem = function (text, value) {
        ...
        this.hidden.value = $(this.select).find('option:selected').text();
    };
